...
new_user_ratings = complete_ratings_data.filter(lambda x: x[0]==new_user_ID).collect()
...
t0 = time()
new_ratings_model = ALS.train(complete_ratings_data, best_rank, seed=seed, 
                              iterations=iterations, lambda_=regularization_parameter)
tt = time() - t0

print ("New model trained in %s seconds" % round(tt,3))

new_user_ratings_ids = list(new_user_ratings_RDD.map(lambda x: x[1]).collect()) # get just movie IDs
